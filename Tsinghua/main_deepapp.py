...
from model.DeepApp import AppPreUserGtm
...
def main():
    ...
    # model & optimizer
    model = AppPreUserGtm(num_apps, num_users, args.dim)
    model.to(device)
    ...
    for e in range(args.epoch):
        ...
        for batch in train_loader:
            ...
            optimizer.zero_grad()
            loss = model(time_vecs, app_seqs, users, targets, 'train')
            loss.backward()
            optimizer.step()
        ...
        with torch.no_grad():
            for batch in test_loader:
                ...
                scores = model(time_vecs, app_seqs, users, targets, 'predict')
