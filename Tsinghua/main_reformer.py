...
from model.Reformer import AppUsageReformer
...
def main():
    ...
    # model & optimizer
    configs = {
        'app_vocab_size': num_apps,      # number of unique apps
        'time_feat_dim': 8,         # one-hot or embedding dim of time features
        'd_model': args.dim,
        'n_heads': 4,
        'd_ff': 128,
        'dropout': 0.1,
        ...
        'pred_len': 1,
        'num_app': num_apps,             # number of app classes to predict
    }
    
    model = AppUsageReformer(configs)
    model.to(device)
